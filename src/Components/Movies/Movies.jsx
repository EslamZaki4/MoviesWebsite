  ...
  return (
    <>
      <div className='container mt-5'>
        <Row >
          {movies.map((prd) => (
            <div key={prd.id} className="col card-parent">
              <div style={{  position:"relative" ,marginTop:"50px", height: '300px' }}>
                
                <div  className='rating-icon'> <span className='rate'> {prd.vote_average}</span> </div>
                  <BsFillHeartFill
                    onClick={() => {
                      addtoFav({
                        id: prd.id,
                    ...
                    left:"5px" ,
                     fontSize: "21px",
                     color: isMovieInFavorites(prd.id) ? "red" : "white", 
                    cursor: "pointer"}}
                  />
